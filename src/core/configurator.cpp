...
void Core::importer(const std::string& path) {
    ...
    std::string collectionPath = stc::cwmm::ram_collection();
    std::string presetPath = stc::cwmm::ram_preset();
    ...
    std::map<std::string, std::string> renamedList;
    {
        ...
        for (const auto* entry : archive) {
            ...
            if (archivedFilePath.compare(0, collectionPath.size(), collectionPath) == 0) {
                if (std::filesystem::exists(archivedFilePath)) {
                    ...
                    mainCollectionFile = file_renaming(stc::string::get_name(archivedFilePath, dir, expansion), Core::lang["LANG_LABEL_RENAME_COLLECTIONS"]);
        ...
        if (error) { // This block will be executed only if no collection file is found in the archive.
    ...
    //////////////////////////////////////////////////////////////////////////////////////////
    // This code module will only exist here until the wmml library is modified
    ...
    std::vector<Core::wmmb> modlist = parser(mainCollectionFile);
    std::vector<Core::wmmb> importList;
    ...
    for (const auto* entry : archive) {
        ...
        if (targetFilename.compare(0, collectionPath.size(), collectionPath) == 0 ||
            targetFilename.compare(0, presetPath.size(), presetPath) == 0)
            continue;
        else if (targetFilename.compare(0, modsDirectory.size(), modsDirectory) == 0) {
